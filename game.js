                ...
                if (this.field[x][y] == ' ') {
                    ...
                    for (var dx = -1; dx <= 1; dx++) {
                        for (var dy = -1; dy <= 1; dy++) {
                            var x0 = x + dx;                                                         //dealing with offset
                            var y0 = y + dy;
                            if (x0 >= 0 && x0 < width && y0 >= 0 && y0 < height)
                                count += (this.field[x0][y0] == 'x' ? 1 : 0);                        //if equal to x , 1, else 0
                        }
                    }
        ...
        for (var y = 0; y < this.height; y++) {
            for (var x = 0; x < this.width; x++) {
                if (!this.veil[x][y])
                    ...
                    /*output += im;*/
                    document.getElementById("output").appendChild(im);
            }
            output += "\n";
        }
        return output;
